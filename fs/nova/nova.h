...
/*
	...
	node = kmalloc(cpu_num*sizeof( struct tail_pool *), GFP_KERNEL);
	for(i=0 ; i<cpu_num ; i++){
		if(sih->tail_queue[i].cnt < 10){
			//nova_dbg("jh dbg: insert queue cpu:%d cnt:%d\n", i, sih->tail_queue[i].cnt);
			node[i] = kmalloc(sizeof(struct tail_pool), GFP_KERNEL);			
			node[i]->next = node[i]->prev = NULL;
			node[i]->tail_addr = nova_get_append_head(sb, pi, sih, sih->log_tail, entry_size, MAIN_LOG, 0, &extended);
			sih->log_tail = node[i]->tail_addr + entry_size;

			if(sih->tail_queue[i].head == NULL)
				sih->tail_queue[i].head = sih->tail_queue[i].tail = node[i];
			else{
				curr = sih->tail_queue[i].head;
				node[i]->next = curr;
				curr->prev = node[i];						
				sih->tail_queue[i].head = node[i];
			}
			//TODO: change to atomic oper
...
/*
	...
	nova_dbg("jh dbg: pop cpu:%d\n", cpuid);
	
	curr = sih->tail_queue[cpuid].tail;	
	if(sih->tail_queue[cpuid].head == sih->tail_queue[cpuid].tail)
		sih->tail_queue[cpuid].head = sih->tail_queue[cpuid].tail = NULL;
	else{	
		curr->prev->next = NULL;
		sih->tail_queue[cpuid].tail = curr->prev;
	}
	//TODO: change to atomic oper
...
static inline void clear_tail_queue(struct nova_inode_info_header *sih)
{
	int i;
	struct tail_pool *curr;
	struct tail_pool *del;
	for(i=0; i<cpu_num; i++){
		curr = sih->tail_queue[i].tail;
		while(curr != NULL){
			del = curr;			
			curr = del->prev;
			kfree(del);
...
static __always_inline long nova_atomic64_add_return(long i, u64 *tail)
{
	return i + xadd(tail, i);
}
